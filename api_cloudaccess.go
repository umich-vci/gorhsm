...
func (a *CloudaccessAPIService) AddProviderAccountsExecute(r ApiAddProviderAccountsRequest) (*http.Response, error) {
	...
	localVarPath = strings.Replace(localVarPath, "{"+"ProviderShortName"+"}", url.PathEscape(parameterValueToString(r.providerShortName, "providerShortName")), -1)
...
type ApiRemoveProviderAccountBySourceIDRequest struct {
	ctx        context.Context
	ApiService *CloudaccessAPIService
	sourceID   *string
}

// source id with which we have to delete the provider account
func (r ApiRemoveProviderAccountBySourceIDRequest) SourceID(sourceID string) ApiRemoveProviderAccountBySourceIDRequest {
	r.sourceID = &sourceID
	return r
}

func (r ApiRemoveProviderAccountBySourceIDRequest) Execute() (*http.Response, error) {
	return r.ApiService.RemoveProviderAccountBySourceIDExecute(r)
}

/*
RemoveProviderAccountBySourceID Remove a provider account by source id

Removes a currently-enabled provider account, including removing access to Gold Images and associated source details, where applicable using the sourceID provided. You can find a list of currently-enabled provider accounts and provider short names from the `/v1/cloud_access_providers/enabled` endpoint.

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiRemoveProviderAccountBySourceIDRequest
*/
func (a *CloudaccessAPIService) RemoveProviderAccountBySourceID(ctx context.Context) ApiRemoveProviderAccountBySourceIDRequest {
	return ApiRemoveProviderAccountBySourceIDRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
// Execute executes the request
func (a *CloudaccessAPIService) RemoveProviderAccountBySourceIDExecute(r ApiRemoveProviderAccountBySourceIDRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod = http.MethodDelete
		localVarPostBody   interface{}
		formFiles          []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "CloudaccessAPIService.RemoveProviderAccountBySourceID")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/cloud_access_providers/accounts"
